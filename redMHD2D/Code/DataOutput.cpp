...
DataOutput::DataOutput(Input *input, int Nx, int Nky, double Lx, double Ly, double *X, double *Psi0, double Viscosity, double Resistivity) 
{
  ...
  //check(H5LTset_attribute_double(initGroup, ".", "a", &a, 1 ), DMESG("Write Constants Attributes"));
  //check(H5LTset_attribute_double(initGroup, ".", "b", &b, 1 ), DMESG("Write Constants Attributes"));
  //check(H5LTset_attribute_double(initGroup, ".", "c", &c, 1 ), DMESG("Write Constants Attributes"));
  ...
  // Energy evolution
  {
    hid_t energyGroup = check(H5Gcreate(fileOutput, "/Energy",H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT), DMESG("Error creating group file for Phasespace : H5Gcreate"));
  
    hsize_t dim[]    = { 1 };
    hsize_t maxdim[] = { H5S_UNLIMITED };
  
    FA_Energy_Phi  = new FileAttr("Phi" , powerGroup,  1,   dim,     maxdim, dim, offset0, dim, offset0, 1); 
    FA_Energy_Psi  = new FileAttr("Psi" , powerGroup,  1,   dim,     maxdim, dim, offset0, dim, offset0, 1); 
    FA_Energy_Time = new FileAttr("Time", powerGroup,  1, time_dim, timing_maxdim, timing_chunkdim, offset0, timing_chunkdim, offset0, 1, timing_tid); 
...
DataOutput::~DataOutput() {
    ...
    delete FA_Energy_Phi, delete FA_Energy_Psi, delete FA_Energy_Time;
...
void DataOutput::output(int Step, double Time, double dt, int Nky, int Nx, cmplxd Phi[Nky][Nx], cmplxd Psi[Nky][Nx]) 
{
  ...
  if(((fmod(Time+dt, OutputTimePower) < dt))) {
    ...
    // Could be done in post-processing but this is more clear
    double phi_energy=0., psi_energy=0.;

    for(int y_k =0; y_k < Nky; y_k++) { 
      phi_energy += PowerKy_Phi[y_k] * PowerKy_Phi[y_k];
      psi_energy += PowerKy_Psi[y_k] * PowerKy_Psi[y_k];
    }

    FA_Energy_Phi ->write(&phi_energy);
    FA_Energy_Psi ->write(&psi_energy);
    FA_Energy_Time->write(&T);
    
    std::cout << " Writing Power - phi : " << phi_energy << " psi : " << psi_energy <<  std::endl;
